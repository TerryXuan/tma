...
object TestMain {
  def main(args: Array[String]): Unit = {
    ...
    val gene = spark.read.option("header", value = true).option("inferSchema", value = true).csv("E:/data/TMA/dataset/gene")
    ...
    spark.sparkContext.parallelize(1 to 1000).map(i => {
      ...
      resultIndex.mkString(",")
    }).toDF().write.mode("overwrite").csv("E:/data/TMA/dataset/chromosome1")
    ...
    //      print("\r" + i)
    //      if (i % 10000 == 0 && i != 0) {
    //        spark.sparkContext.parallelize(rdd, numSlices = 1).toDF().write.mode("append").csv("E:/data/TMA/dataset/chromosome1")
    //        rdd.clear()
    //        println(rdd.length)
    //      }
    //    }
